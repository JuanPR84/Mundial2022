...
for($i=1;$i<=48;$i++){
	...
	if(($_POST['resultado1-'.$i]!='')and($_POST['resultado2-'.$i]!='')){
		...
		$cons = new base_datos;
		$cons->connect();
		$sql = "Select * from fechas where anno = '$anno' and id='$i'";
		$res = $cons->query($sql);
		...
		if ($estadop == 'ABIERTO'){
			...
			$cons = new base_datos;
			$cons->connect();
			$sql = "Select * from pronostico where id_partido = '$i' and fase = 'Primera Fase' and usuario = '$iu'";
			$res = $cons->query($sql);
			while($dato = $cons->fetch_row($res))	{
			...
			if ($cantpuntos > 0){
				...
				$sql = "UPDATE pronostico SET golesequipo1 = '$goles1', golesequipo2 = '$goles2', equipoganador = '$ganador', primergol = '$primergol' where id_partido = '$i' and fase = 'Primera Fase' and usuario = '$iu'";
				
			} else {
				
				$sql = "INSERT INTO pronostico (id_partido,golesequipo1,golesequipo2,fase,anno,usuario,puntosobtenidos,equipoganador,primergol) VALUES ('$i','$goles1','$goles2','Primera Fase','$anno','$iu','0','$ganador','$primergol')";
			}
...
echo "<script>
		alert('Marcadores grabados con exito hasta 1 hora antes de cada partido');
